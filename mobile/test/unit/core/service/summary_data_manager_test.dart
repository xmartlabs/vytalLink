...
void main() {
  ...
  group('SummaryDataManager', () {
    ...
    test('supports HRV metrics with average statistic', () async {
      final start = DateTime(2025, 1, 1);
      final end = DateTime(2025, 1, 15);
        ...
        SummaryRequest(
          ...
          metrics: const [
            SummaryMetricRequest(
              valueType: VytalHealthDataCategory.HRV,
            ),
          ],
        ),
      );
      ...
      final request = captured.single as HealthDataRequest;

      expect(request.valueType, VytalHealthDataCategory.HRV);
      expect(request.statistic, StatisticType.average);
      expect(request.groupBy, TimeGroupBy.day);
    });
  });
}
